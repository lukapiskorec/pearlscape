...
def _add_section_title_strip(page, code, plane_index, legend, section_size):
    ...
    scale, total bead count, and a colour-swatch + count legend. `legend` is the
    [((r,g,b), count), ...] list from sections.color_counts."""
    ...
    # Left block: code, scale, bead total.
    _page_text(page, layer_idx, f"{code}  /  C{plane_index:03d}", 16, 46, 20)
    _page_text(page, layer_idx, "Scale 1:1", 16, 24, 12)
    _page_text(page, layer_idx, f"{total} beads", 130, 24, 12)

    # Legend: swatch + count, laid left-to-right from x0, wrapping up a row.
    sw = 22.0          # swatch side
    slot = 96.0        # horizontal pitch per entry
    x0 = 250.0
    rows_y = (44.0, 14.0)   # two rows inside the strip
    per_row = max(1, int((w - x0 - 8.0) / slot))
    for i, (rgb, n) in enumerate(legend):
        col = i % per_row
        row = (i // per_row) % len(rows_y)
        x = x0 + col * slot
        y = rows_y[row]
        _page_filled_rect(page, layer_idx, x, y, sw, sw,
                          sd.Color.FromArgb(rgb[0], rgb[1], rgb[2]), hatch_idx, tol)
        doc.Objects.AddPolyline(
            rg.Polyline([rg.Point3d(x, y, 0), rg.Point3d(x + sw, y, 0),
                         rg.Point3d(x + sw, y + sw, 0), rg.Point3d(x, y + sw, 0),
                         rg.Point3d(x, y, 0)]),
            _page_attrs(page, layer_idx, _BORDER_RGB))
        _page_text(page, layer_idx, f"x{n}", x + sw + 6, y + 3, 13)
